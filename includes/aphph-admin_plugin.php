...
class Aphph_Admin
{
	...
	*/
	...
	{
		$contents =  file_get_contents(APHPH_PLUGIN_PATH . APHPH_DS . 'includes/prism/conponents_json.json');
					...
					<table class="form-table">
						...
						<tr>
							...
							<td>
								...
								<p class="description">Used when "Line Numbers" plugin is checked</p>
							</td>
						</tr>
						<tr>
							<th>Max Height</th>
...
@media (min-width:641px) and (max-width:800px) {
	...
	public function option_language_list() {
		...
		{
			// AFallback to old plugin version
		...
		foreach ($load_lang as $lang_name => $lang_title) {
			...
			if (substr($lang_name, 0, 3) != 'add' 
					&& !file_exists(APHPH_PLUGIN_PATH . APHPH_DS . 'includes' . APHPH_DS . 'prism' . APHPH_DS . 'components' . APHPH_DS . $file_name[$lang_name])
				)
			{
			...
			if (is_array($lang_list[$lang_name])) {
	...
	public function option_plugin_options() 
	{
		...
		foreach($this->components_list['plugins'] as $plugin_name => $item) {
			if ($plugin_name == 'meta')
				continue;
			
			$title = is_array($item) ? $item['title'] : $item?>
			<p>
				<label for="aphph-<?=$plugin_name?>">
					<?php $checked = $this->options[$plugin_name] == 1 ? ' checked="checked"' : ''; ?>
					<input type="checkbox" name="<?php echo APHPH_OPTION?>[<?php echo $plugin_name?>]" id="aphph-<?php echo $plugin_name?>" value="1"<?php echo $checked?>/>
					<?php echo $title ?>
				</label>
			</p>
		<?php }
	...
	{
		$obj = new Aphph_Build;
		$obj->build_files( $this->components_list );
	}
}
