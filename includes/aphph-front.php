		...
		foreach ($matches[1] as $tag_index => $pre_tag)
		{
			...
			*/
			...
			foreach ($exp as $key => $class_item)
			{
				...
				if ($class_item != 'wp-block-aphph-prism-block' && $param != 'lang' && $param != 'class') {
					$pretag_class['add_' . $key] = $class_item;
				}
				// gutter
				elseif ($param == 'gutter')
				{
			...
			*/
			$content = preg_replace('/('.$pre_tag.')(.*)(<\s*\/pre\s*>)/isU', 
									$new_pre_tag . '<code rel="'.$language.'" class="'.$codetag_class.'">$2</code></pre>',
									$content);
		}
		return $content;	
	}
