...
class Aphph_Front 
{
	...
	public function alter_content($content, $matches)
	{
		...
		if ($this->options['gutter'] || $this->options['line-highlight']) {
			...
			foreach ($exp as $key => $class_item)
			{
				...
				if ($param == 'lang') {
					if (strpos($value, 'add') !== false) {
						...
						$pretag_class['language-' . $value] = 'language-' . $value;
					}
				}
				...
				// gutter
				elseif ($param == 'gutter' || $param == 'line-highlight')
				{
					if ($value == 'true')
					{
...
// echo '<pre>'; print_r($pretag_class);die;
